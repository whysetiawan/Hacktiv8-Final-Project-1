...
type TodoService interface {
	...
	GetTodoByID(id uint) (models.TodoModel, error)
...
func (s *todoService) GetAllTodos() (models.TodoModel, error)  {
	todo, err := s.todoRepository.GetAllTodos()
    if err != nil {
        return todo, err
    }
...
func (s *todoService) GetTodoByID(id uint) (models.TodoModel, error)  {
	todo, err := s.todoRepository.GetTodosByID(uint(id))
	if err != nil {
		return todo, err
	}
	if todo.TodoId == 0 {
